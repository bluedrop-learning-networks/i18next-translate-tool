...
type TranslationFunction = (text: string, sourceLanguage: string, targetLanguage: string) => Promise<string>;
...
export async function translateI18NextJson(
  ...
  for (const [key, value] of Object.entries(sourceJson)) {
    if (typeof value === 'string') {
      ...
      targetJson[key] = await translateI18NextJson(value, sourceLanguage, targetLanguage, translateFn);
    } else {
...
export function createTranslationPrompt(text: string, sourceLanguage: string, targetLanguage: string): string {
...
export async function dummyTranslate(text: string, sourceLanguage: string, targetLanguage: string): Promise<string> {
